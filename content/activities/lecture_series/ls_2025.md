...
- **Talk 2:**
  - **Abstract:** The talk explores how patterns in the digits of real numbers relate to the geometric size of the sets they form on the unit interval. We begin with the \(r\)-adic transformation \(T(x) = rx \pmod{1}\). By the ergodic theorem, almost every number is normal in base \(r\); that is, each digit appears equally often. We then present Eggleston's theorem, which describes the Hausdorff dimension of the set of numbers whose digits appear with fixed frequencies \(p_0, \ldots, p_{r-1}\): \(\dim_H M = -\frac{\sum_i p_i \log p_i}{\log r}\). This shows how simple rules in number expansions can determine precise geometric features of the resulting sets. We will briefly review the idea of Hausdorff dimension, following Sabartha Sarkar's talk, and discuss examples such as the Cantor set.
